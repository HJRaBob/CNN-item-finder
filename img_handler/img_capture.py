...
def capture_object(hwnd2):
    DIR = 'img/object'
    number = len([name for name in os.listdir(DIR) if os.path.isfile(os.path.join(DIR, name))])
    bbox1 = win32gui.GetWindowRect(hwnd2)
    img = ImageGrab.grab(bbox1)
    img_np = np.array(img)
    cv.imwrite(DIR+'/object_{}.png'.format(number),img_np) #saved BGR
...
class Capture_manager(QWidget):
    ...
    def initUI(self):                       
        ...
        self.start_btn = QPushButton('background',self)
        self.start_btn.move(45, 65)

        self.start_btn.clicked[bool].connect(self.start_capture)

        self.object_btn = QPushButton('object',self)
        self.object_btn.move(125, 65)
        self.object_btn.clicked[bool].connect(self.object_capture)
    ...
    def object_capture(self, pressed):
        for i in range(10):
            capture_object(find_window())
            time.sleep(0.3)
